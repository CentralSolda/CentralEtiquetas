...
class Layout75x35:
    ...
    margem_interna: float = 2.5 * mm
    altura_cabecalho: float = 12 * mm
    altura_codigo: float = 5.7 * mm
    ...
    def _desenhar_cabecalho(self, c, x, y):
        ...
        c.rect(x, y + 23 * mm, self.largura, self.altura_cabecalho, fill=1, stroke=0)
        c.setFillColorRGB(*self.cor_laranja)
        c.rect(x, y + 22.2 * mm, self.largura, 0.8 * mm, fill=1, stroke=0)
        c.rect(x, y + 23 * mm, 1.8 * mm, self.altura_cabecalho, fill=1, stroke=0)

    def _desenhar_logo(self, c, x, y, logo_reader: ImageReader):
        logo_x, logo_y = x + 3 * mm, y + 25.2 * mm
        c.setFillColorRGB(*self.cor_branco)
        c.roundRect(logo_x - 0.7 * mm, logo_y - 0.5 * mm, 32.5 * mm, 8.8 * mm, 1.5 * mm, fill=1, stroke=0)
        c.drawImage(logo_reader, logo_x, logo_y, width=31 * mm, height=7.8 * mm, preserveAspectRatio=True, mask="auto")

    def _desenhar_preco(self, c, x, y, produto: dict):
        valor = Decimal(str(produto.get("preco", "0")))
        preco = formatar_brl(valor)
        price_size = 20
        while price_size > 17 and stringWidth(preco, "Helvetica-Bold", price_size) > 38 * mm:
            price_size -= 0.5
        c.setFillColorRGB(*self.cor_branco)
        c.setFont("Helvetica-Bold", price_size)
        c.drawRightString(x + self.largura - 2.5 * mm, y + 27.4 * mm, preco)
    ...
    def _desenhar_descricao(self, c, x, y, produto: dict):
        ...
        first_y = y + (18.9 * mm if len(linhas) == 1 else 19.3 * mm)
        for idx, linha in enumerate(linhas[:2]):
            c.drawString(x + 3 * mm, first_y - idx * 3.0 * mm, linha)

        c.setFillColorRGB(*self.cor_laranja)
        c.roundRect(x + 3 * mm, y + 13.4 * mm, 12 * mm, 0.8 * mm, 0.4 * mm, fill=1, stroke=0)
    ...
    def _quebrar_descricao(self, texto: str, max_width: float):
        for size in [8.6, 8.4, 8.2, 8.0, 7.8, 7.6, 7.4, 7.2]:
            ...
            if ok:
                return lines, size
        return [texto[:26] + ("..." if len(texto) > 26 else "")], 7.2

    def _desenhar_codigo_barras(self, c, x, y, produto: dict):
        sku = str(produto.get("sku", "")).strip()
        base = code128.Code128(sku, barHeight=5.7 * mm, barWidth=0.48 * mm, humanReadable=False)
        scale = min(1.0, 62 * mm / base.width) if base.width else 1.0
        barcode = code128.Code128(sku, barHeight=5.7 * mm, barWidth=0.48 * mm * scale, humanReadable=False)
        c.setFillColorRGB(0, 0, 0)
        barcode.drawOn(c, x + (self.largura - barcode.width) / 2, y + 6.1 * mm)

    def _desenhar_sku(self, c, x, y, produto: dict):
        sku = str(produto.get("sku", "")).strip()
        capsule_w = max(20 * mm, stringWidth(sku, "Helvetica-Bold", 10) + 9 * mm)
        c.setFillColorRGB(*self.cor_azul)
        c.roundRect(x + (self.largura - capsule_w) / 2, y + 1.2 * mm, capsule_w, 4.2 * mm, 2 * mm, fill=1, stroke=0)
        c.setFillColorRGB(*self.cor_branco)
        c.setFont("Helvetica-Bold", 10)
        c.drawCentredString(x + self.largura / 2, y + 2.35 * mm, sku)
