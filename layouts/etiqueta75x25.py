...
class Layout75x25:
    ...
    def _desenhar_cabecalho(self, c, x, y):
        ...
        c.rect(x, y + 14.3 * mm, 1.6 * mm, self.altura_cabecalho, fill=1, stroke=0)

    def _desenhar_logo(self, c, x, y, logo_reader: ImageReader):
        logo_x, logo_y = x + 2.8 * mm, y + 16.6 * mm
        c.setFillColorRGB(*self.cor_branco)
        c.roundRect(logo_x - 0.6 * mm, logo_y - 0.4 * mm, 30.5 * mm, 7.2 * mm, 1.4 * mm, fill=1, stroke=0)
        c.drawImage(logo_reader, logo_x, logo_y, width=29.2 * mm, height=6.4 * mm, preserveAspectRatio=True, mask="auto")

    def _desenhar_preco(self, c, x, y, produto: dict):
        valor = Decimal(str(produto.get("preco", "0")))
        preco = formatar_brl(valor)
        price_size = 15.2
        while price_size > 11.5 and stringWidth(preco, "Helvetica-Bold", price_size) > 38 * mm:
            price_size -= 0.4
        c.setFillColorRGB(*self.cor_branco)
        c.setFont("Helvetica-Bold", price_size)
        c.drawRightString(x + self.largura - 1.5 * mm, y + 18.2 * mm, preco)
    ...
    def _desenhar_descricao(self, c, x, y, produto: dict):
        ...
        for idx, linha in enumerate(linhas[:2]):
            c.drawString(x + 2 * mm, first_y - idx * 3.0 * mm, linha)

    def _quebrar_descricao(self, texto: str, max_width: float):
        for size in [7.4, 7.2, 7.0, 6.8, 6.6, 6.4, 6.2, 6.0]:
            words = texto.split()
            lines = [""]
            ok = True
            for word in words:
            ...
            if ok:
                return lines, size
        return [texto[:26] + ("..." if len(texto) > 26 else "")], 6.0

    def _desenhar_sku(self, c, x, y, produto: dict):
        sku = str(produto.get("sku", "")).strip()
        sku_text = sku
        c.setFillColorRGB(*self.cor_azul)
        capsule_w = max(14 * mm, stringWidth(sku_text, "Helvetica-Bold", 8.2) + 6 * mm)
        capsule_x = x + (self.largura - capsule_w) / 2
        c.roundRect(capsule_x, y + 1.0 * mm, capsule_w, 3.7 * mm, 1.7 * mm, fill=1, stroke=0)
        c.setFillColorRGB(*self.cor_branco)
        c.setFont("Helvetica-Bold", 8.2)
        c.drawCentredString(x + self.largura / 2, y + 1.95 * mm, sku_text)
